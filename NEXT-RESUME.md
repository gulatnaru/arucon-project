...
2026-09-20 · feature/arucon-mobile-autonomous · 시작 HEAD `0e795d9`. SDK55 마이그레이션 체크포인트를 현재 feature 브랜치에 commit/push했으며, 재개 시 실제 HEAD와 origin tracking을 확인한다.
...
3. 이번239/239, lint/typecheck, doctor20/20, 두 JS bundle, CNG25/25 및 native compile/install/process 시작은 실행 증거다. 미래 변경의 fresh 결과로 재사용하지 않는다.
...
- `npm start`로 Metro를 켜고 Simulator에서 개발 앱을 수동 선택하거나 허용된 UI 자동화로 연결한다. 이번에 켠 Metro는 종료했다.
- 자동화/접근성·화면 기록 권한이 준비되면 합성 온보딩, GLB 로딩/모션·터치, 위젯 OS 렌더, 저장/lifecycle를 검증한다. 현재 process start는 이 동작들의 PASS가 아니다.
