...
### 2026-09-20 SDK55 마이그레이션 최신 부록

Expo 55.0.31/RN 0.83.10/React 19.2.0으로 의존성을 정렬하고 SDK55 CNG를 재생성했다. fresh tests 239/239, lint/typecheck PASS, `expo-doctor` 20/20, Android/iOS JS bundle PASS, native generation 25/25. iOS native compile/install PASS 및 별도 `simctl launch` 프로세스 실행 확인 PASS. `expo run:ios` 전체 명령은 마지막 GUI 활성화 단계에서 System Events 권한 부족으로 exit 1 (`BLOCKED_ENV_AUTOMATION_PERMISSION`). CUA Accessibility/Screen Recording 권한 부재로 실제 GLB/터치/모션과 WidgetKit 화면은 `NOT_EVALUATED`; physical device `NOT_RUN`; Android native `BLOCKED_ENV`. 기존 APP-06 숫자는 당시 증거로 유지한다.

기준일: 2026-09-19 KST의 APP-06 실행 기록이며, SDK57 네이티브 실패 이력을 아래에 보존했다. 현재 SDK55 결과는 맨 위 최신 부록을 따른다. 체크포인트 `3b52a5b`에서 기존 구현을 유지하여 APP-05/06을 재개한 모바일 개발 후보다. 과거 별도 `mobile` 저장소의 55개 테스트·Windows 빌드 기록은 이 소스의 증거로 재사용하지 않는다. 전체 판정은 **PARTIAL**이며 설치형 앱 실행과 MVP 출시 완료를 뜻하지 않는다.

### 2026-09-20 SDK57 네이티브 실패 이력 (SDK55 전환 전)

Xcode 26.3, iOS 26.3 iPhone 16e Simulator, Swift 6.2.4, CocoaPods 1.17.0을 확인했다. 위젯 `AruconWidget.swift`는 실제 xcodebuild에서 컴파일·링크됐지만 앱 빌드는 `JavaScriptRuntime.swift` Swift 동시성 오류로 exit 65였다. Expo SDK 57의 공식 Xcode 26.4 이상 요구에도 미달하므로 당시 앱 native compile은 **BLOCKED_ENV**였다. Simulator 앱 실행, widget OS 렌더링, physical device 검증은 **NOT_RUN**이다. Android SDK/adb/emulator도 없다. 기존 77/77 및 번들 결과는 2026-09-19 실행 증거이며 이번 native 실패를 덮어쓰지 않는다.

## APP-06 및 SDK57 당시 실행 환경과 증거
...
- 당시 APP-06 실행 시점에는 Xcode/Simulator와 Android SDK/adb가 없었다. SDK55 전환 전 SDK57 재검증에서는 Xcode/Simulator가 확인됐지만 앱 native compile이 exit 65로 실패했다. 그 당시 네이티브 개발 빌드·앱 실행·위젯 OS 렌더링은 **BLOCKED_ENV / NOT_RUN**이며 실제 앱 캡처·영상·FPS·Expo SQLite 네이티브 실행·APK/IPA도 **NOT_RUN / 확인 불가**다. Metro export는 JavaScript/자산 번들 검사다.
