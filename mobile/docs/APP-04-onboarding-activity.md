...
합성 테스트는 이름·연령 경계·동의 미검증, 상태 의미, 입력 유효성, 공급자 혼합 차단, 중복·역순 revision, APP-02 활동 정산 연결 후 EXP/체력 불변을 검사한다. `ActivityStatusView`와 `DevOnboardingScreen`은 앱에 아직 연결되지 않았다. 네이티브 렌더·OS 백그라운드·실기기 권한 및 실제 건강 기록은 확인하지 않았다(HS-01/02).
