...
Operational retry limits, API error codes, retention/deletion, active-device transfer, forced recovery UX, backup format, and automatic handling of an old device remain `DecisionRequired`. The code therefore accepts writer identity and fake server policy explicitly and never chooses production defaults.

## Implemented local contract

- SQLite schema v5 retains the additive v3 `local_outbox` status fields (`pending / synced / conflict / error`), attempt/error and acknowledgement metadata, and the writer identity captured at local commit. V4 stores the originating config version directly on the outbox row so non-domain ledgers can use the same queue; v5 adds a caller-prepared one-time resolution ledger. Existing rows are preserved and backfilled as `pending`; an identity is never invented for them.
...
- `SyncCoordinator` performs one caller-requested batch. It has no built-in loop, timer, retry cap, or backoff policy.
...
- Schema migration is transactional. A v2→v5 injected failure leaves `user_version`, snapshot bytes, and old table shape unchanged; a later retry upgrades and preserves the row. A separate v4→v5 injected failure rolls marker back while preserving the pre-existing resolution record, then clean retry backfills the marker. Unsupported future versions still fail without replacement.
...
| AT-SYNC-02 server commit then response loss | first pass records retryable error; same `actionId` returns the original ack; fake committed count stays one | production endpoint and error mapping are DEC-17/external |
...
| AT-SYNC-06 unsupported/order/version boundaries | config allowlist, strict injected local order, writer epoch, and per-pet acknowledgement reuse checks exist as fake/local boundaries | retry/error/version support policy remains OPEN |
...
| AT-DATA-03 migration failure | injected v2→v5 and v4→v5 failures preserve prior schema/data and clean retry succeeds | file backup/restore operations remain DEC-17 |
