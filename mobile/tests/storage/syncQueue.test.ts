...
test('offline local commit remains pending until one server acknowledgement and never re-awards EXP', async () => {
  ...
  const actions = await queue.listDispatchable(10);
  ...
  const result = await new SyncCoordinator(queue, synthetic).flushOnce(10);
  ...
  assert.deepEqual(await queue.listDispatchable(10), []);
...
test('response loss after server commit is retried by actionId and resolves to the original acknowledgement', async () => {
  ...
  const coordinator = new SyncCoordinator(queue, synthetic);

  assert.deepEqual(await coordinator.flushOnce(10), { attempted: 1, acknowledged: 0, retryableErrors: 1, conflicts: 0 });
  assert.equal(synthetic.committedActionCount(), 1);
  assert.equal((await queue.summary('pet-1')).status, 'error');
  assert.equal((await store.loadPet('pet-1')).totalExpUnits, 15_000_000);

  assert.deepEqual(await coordinator.flushOnce(10), { attempted: 1, acknowledged: 1, retryableErrors: 0, conflicts: 0 });
  assert.equal(synthetic.committedActionCount(), 1);
  assert.equal((await store.loadPet('pet-1')).totalExpUnits, 15_000_000);
...
test('synthetic inactive writer epoch becomes a preserved conflict and is not auto-merged or resent', async () => {
  ...
  const coordinator = new SyncCoordinator(inactiveQueue, server());
  const before = await store.loadPet('pet-1');
  assert.deepEqual(await coordinator.flushOnce(10), { attempted: 1, acknowledged: 0, retryableErrors: 0, conflicts: 1 });
  assert.deepEqual(await store.loadPet('pet-1'), before);
  assert.deepEqual(await inactiveQueue.listDispatchable(10), []);
  assert.deepEqual(await inactiveQueue.summary('pet-1'), {
    status: 'conflict', pendingCount: 0, errorCount: 0, conflictCount: 1,
  ...
  await assert.rejects(inactiveQueue.recordRetryableError('meal-command', 'synthetic_retry'), /DecisionRequired: conflict row/);
...
test('synthetic strict sequence policy rejects out-of-order actions and accepts them in configured order', async () => {
  ...
  const second = { ...base, actionId: 'sequence-2', localSequence: 2, envelope: envelopeFor(2) };
  ...
  const first = { ...base, actionId: 'sequence-1', localSequence: 1, envelope: envelopeFor(1) };
...
test('sync transport rejects copied envelopes that bypass the outbound privacy builder', async () => {
  ...
  await assert.rejects(server().deliver({
    actionId: 'copied', petId: 'pet-1', localSequence: 1, writer,
    configVersion: DEV_GAME_CONFIG.version, envelope: { ...issued },
...
test('sync transport rejects an issued envelope after its consent authority is revoked', async () => {
  ...
  await assert.rejects(server().deliver({
    ...
    configVersion: DEV_GAME_CONFIG.version, envelope,
  }), /Valid DEV outbound envelope required/);
});
...
test('outbox rows created before writer policy configuration are preserved and cannot be uploaded with an invented epoch', async () => {
  ...
  const queue = new SqliteSyncQueue(db, projector());
  await assert.rejects(queue.listDispatchable(10), /DecisionRequired: outbox writer identity is not configured/);
